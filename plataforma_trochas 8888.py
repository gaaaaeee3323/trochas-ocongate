...
def init_db():
    ...
    for col_def in [
        ...
        try:
            ...
            pass
    for col_def in ["componente TEXT", "prioridad TEXT", "foto_evidencia BLOB"]:
        try:
            c.execute(f"ALTER TABLE intervenciones ADD COLUMN {col_def}")
        except sqlite3.OperationalError:
            pass
    conn.commit()
    conn.close()
...
def insertar_intervencion(data: dict):
    ...
    c.execute("""
        INSERT INTO intervenciones (tramo_id, dano_id, tipo_mantenimiento, componente, prioridad,
            actividades, expediente, fecha_programada, duracion_dias, costo_estimado, estado,
    ...
    """, (
        ...
        data["duracion_dias"], data["costo_estimado"], data["estado"], data["responsable"],
        data["observaciones"], data.get("foto_evidencia"),
    ))
    conn.commit()
    conn.close()
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    st.caption("Programa y consulta las intervenciones según los tipos de mantenimiento y componente vial afectado.")
    ...
    if tramos.empty:
        ...
        tramo_default_id = st.session_state.get("ultimo_dano_tramo")
        ...
        with st.form("form_programacion", clear_on_submit=True):
            ...
            tramo_sel_dict = tramos[tramos["id"] == tramo_id_sel].iloc[0]
            c2.text_input("Ubicación del tramo", value=f"{tramo_sel_dict['comunidad']}, {tramo_sel_dict['distrito']} ({tramo_sel_dict['longitud_km']} km)", disabled=True)
            ...
            expediente = c5.text_input("N° de Expediente Técnico", placeholder="Ej: ET-MDO-006")
            fecha_programada = c6.date_input("Fecha programada de inicio", value=date.today())
            duracion_dias = c7.number_input("Duración estimada (días)", min_value=1.0, value=15.0, step=1.0)

            c8, c9, c10 = st.columns(3)
            costo_estimado = c8.number_input("Costo estimado (S/.)", min_value=0.0, step=100.0, value=10000.0)
            estado_int = c9.selectbox("Estado de la intervención", ESTADOS_INTERVENCION)
            prioridad_int = c10.selectbox("Prioridad de intervención *", PRIORIDADES_INTERVENCION,
                                          index=PRIORIDADES_INTERVENCION.index(prioridad_sugerida) if prioridad_sugerida in PRIORIDADES_INTERVENCION else 1)

            responsable_int = st.text_input("Responsable / área encargada",
                                             value="Subgerencia de Gestión de Riesgos y Mantenimiento")
            observaciones_int = st.text_area("Descripción de los trabajos a ejecutar / Observaciones")
            foto_evidencia_int = st.file_uploader("Evidencia fotográfica de la intervención (opcional)",
                                                   type=["jpg", "jpeg", "png"])
        ...
        if guardar_int:
            insertar_intervencion({
                ...
                "observaciones": observaciones_int, "foto_evidencia": img_to_blob(foto_evidencia_int),
            })
            st.success(f"✅ Intervención de **{tipo_mant}** ({componente_int}) programada para el tramo seleccionado.")
        ...
        if intervenciones.empty:
            ...
            tab_cron, tab_tramo, tab_tabla = st.tabs(
                ["📅 Cronograma general", "📍 Vista por tramo", "📋 Tabla completa"])
            ...
            with tab_cron:
                ...
                fig_g = px.timeline(
                    ...
                    hover_data=["expediente", "estado", "componente", "prioridad", "costo_estimado"],
                )
            ...
            with tab_tramo:
                tramo_filtro = st.selectbox("Selecciona un tramo", sorted(intervenciones["codigo_tramo"].dropna().unique()))
                ...
                st.markdown(
                    f"### 📍 {tramo_filtro} — {tramo_info['nombre']}<br>"
                ...
                st.markdown(f"**Estado actual:** {badge_estado(tramo_info['estado_actual'])}", unsafe_allow_html=True)
                st.markdown("###")
                ...
                for _, interv in interv_t.iterrows():
                    ...
                    with col_card:
                        ...
                        📅 Programado: {interv['fecha_programada']} ({interv['duracion_dias']} días) &nbsp;|&nbsp;
            ...
            with tab_tabla:
                st.dataframe(
                    intervenciones[["codigo_tramo", "nombre_tramo", "comunidad_tramo", "longitud_tramo_km",
                                     ...
                                     "ultima_intervencion_tramo", "fecha_programada", "estado", "prioridad",
                                     "costo_estimado", "responsable"]]
                    .rename(columns={
                        ...
                        "ultima_intervencion_tramo": "Última Interv.", "fecha_programada": "F. Programada",
                        "prioridad": "Prioridad", "costo_estimado": "Costo (S/.)",
                    }), use_container_width=True, hide_index=True,
                )
                st.markdown("##### Actualizar estado de una intervención")
                opciones_upd = {f"#{r.id} · {r.codigo_tramo} · {r.tipo_mantenimiento} · {r.fecha_programada}": r.id
                                for r in intervenciones.itertuples()}
                sel_upd = st.selectbox("Intervención", list(opciones_upd.keys()), key="upd_interv")
                nuevo_estado_upd = st.selectbox("Nuevo estado", ESTADOS_INTERVENCION, key="upd_estado")
