...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    st.caption(
        "Planificación técnica de intervenciones: sugerencia automática de fechas, alertas por temporada "
        "de lluvias y criticidad, seguimiento de los 5 tipos de mantenimiento, y cronograma Gantt consolidado."
    )
    ...
    if tramos.empty:
        ...
        st.markdown("---")
        st.markdown("### 1️⃣ Nueva intervención — planificación asistida")
        ...
        # ---- Selección reactiva (fuera del form) para poder calcular la sugerencia en vivo ----
        s1, s2 = st.columns(2)
        sel_tramo_label = s1.selectbox(
        ...
        tramo_sel_dict = tramos[tramos["id"] == tramo_id_sel].iloc[0].to_dict()
        ...
        if guardar_int:
            ...
            st.success(f"✅ Intervención de **{tipo_mant}** ({componente_int}) programada para el tramo {tramo_sel_dict['codigo']}.")
        ...
        if intervenciones.empty:
            ...
            st.markdown("### 2️⃣ Resumen de mantenimiento por tramo")
            ...
            resumen_cols = ["tipo_mantenimiento", "componente", "fecha_programada", "fecha_fin",
                             "duracion_dias", "costo_estimado", "estado", "prioridad"]
            st.dataframe(
                interv_t[resumen_cols].rename(columns={
                    "tipo_mantenimiento": "Tipo de mantenimiento", "componente": "Componente",
                    "fecha_programada": "Inicio tentativo", "fecha_fin": "Fin tentativo",
                    "duracion_dias": "Días", "costo_estimado": "Costo (S/.)", "estado": "Estado",
                    "prioridad": "Prioridad",
                }), use_container_width=True, hide_index=True,
            )

            st.markdown("##### Fichas de planificación detalladas")
            ...
            st.markdown("---")
            st.markdown("### 3️⃣ Tabla completa y actualización de estado")
            st.dataframe(
            ...
            st.markdown("---")
            st.markdown("### 4️⃣ Cronograma Gantt consolidado de mantenimiento vial")
            st.caption("Vista general de todas las intervenciones programadas: qué se ejecuta primero, qué coincide en el tiempo y cuánto dura cada una.")
            ...
            fig_g = px.timeline(
                ...
                hover_data=["componente", "expediente", "estado", "prioridad", "costo_estimado", "duracion_dias"],
                text="actividades",
            )
            fig_g.update_yaxes(autorange="reversed", title="")
            fig_g.update_xaxes(title="Línea de tiempo")
            fig_g.update_traces(textposition="inside", textfont_size=9)
            fig_g.update_layout(height=420, legend_title="Tipo de mantenimiento",
