...
def init_db():
    ...
    for col_def in [
        ...
        try:
            ...
            pass
    for col_def in ["componente TEXT", "prioridad TEXT", "foto_evidencia BLOB", "fecha_fin TEXT"]:
        try:
...
<style>
    ...
    .main-header {
        background: linear-gradient(135deg, #0b2545 0%, #15396b 55%, #0b2545 100%);
        padding: 1.6rem 2rem; border-radius: 8px; color: white; margin-bottom: 1.3rem;
    ...
    .module-header {
        ...
        background: linear-gradient(120deg, #0b2545 0%, #1a3d6e 100%);
        border-radius: 8px; padding: 0.95rem 1.3rem; margin-bottom: 0.4rem;
        border-left: 5px solid #c9a227;
        box-shadow: 0 3px 10px rgba(11,37,69,0.18);
    }
    .module-header .mh-icon { font-size:1.7rem; line-height:1; }
    .module-header .mh-title { color:#fff; font-size:1.35rem; font-weight:700; font-family:'Georgia',serif; margin:0; }
    .module-header .mh-sub { color:#cfd8e8; font-size:0.85rem; margin-top:0.15rem; }

    .kpi-card {
        background: #ffffff; border: 1px solid #e3e7ee; border-top: 4px solid #0b2545;
        border-radius: 8px; padding: 1rem 1.1rem; box-shadow: 0 2px 6px rgba(11,37,69,0.07);
        transition: transform 0.15s ease, box-shadow 0.15s ease;
    }
    .kpi-card:hover { transform: translateY(-2px); box-shadow: 0 6px 16px rgba(11,37,69,0.14); }
    ...
    .badge-bueno { background:#dcedc8; color:#1b5e20; padding:3px 12px; border-radius:14px; font-weight:700; font-size:0.78rem;}
    .badge-regular { background:#fff3cd; color:#7a6500; padding:3px 12px; border-radius:14px; font-weight:700; font-size:0.78rem;}
    .badge-malo { background:#ffe0b2; color:#a04b00; padding:3px 12px; border-radius:14px; font-weight:700; font-size:0.78rem;}
    .badge-muymalo { background:#f5c6cb; color:#7a0c14; padding:3px 12px; border-radius:14px; font-weight:700; font-size:0.78rem;}

    /* Aviso de autocompletado inteligente */
    .auto-fill-note {
        background:#eef6ee; border-left:4px solid #2e7d32; border-radius:6px;
        padding:0.55rem 0.9rem; font-size:0.82rem; color:#1b5e20; margin-bottom:0.7rem;
    }

    section[data-testid="stSidebar"] {
        background: linear-gradient(180deg, #0b2545 0%, #0d2c54 100%);
    }
    section[data-testid="stSidebar"] * { color: #f0f2f6 !important; }
    section[data-testid="stSidebar"] .stRadio label { font-size: 0.92rem; padding: 2px 0; }
    section[data-testid="stSidebar"] hr { border-color: rgba(255,255,255,0.18); }

    div[data-baseweb="tab-list"] { gap: 6px; }
    button[data-baseweb="tab"] {
        font-weight:600; border-radius: 6px 6px 0 0 !important;
    }
    button[data-baseweb="tab"][aria-selected="true"] {
        background-color: #0b254510 !important; border-bottom: 3px solid #c9a227 !important;
    }

    .stButton button[kind="primary"], .stFormSubmitButton button {
        background-color: #0b2545; border: none; font-weight:600;
    ...
    div[data-testid="stExpander"] {
        border: 1px solid #e3e7ee; border-radius: 8px; box-shadow: 0 1px 4px rgba(11,37,69,0.05);
    }
        ...
        if tramos_mant.empty:
            ...
            if guardar_int:
                insertar_intervencion({
                    ...
                    "foto_evidencia": img_to_blob(foto_evidencia_int),
                })
                st.session_state["ultimo_responsable_mant"] = responsable_int
                st.success(f"✅ Intervención de **{tipo_mant}** ({componente_int}) programada para el tramo {tramo_sel_dict['codigo']}. Consulta el cronograma en **🗓️ Programación de Mantenimiento**.")
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if tramos.empty:
        ...
        with exp_col1:
            st.markdown("##### 📷 Evidencia fotográfica")
            foto_bytes_exp, foto_label_exp = _primera_foto_disponible(tramo_dict_ficha)
            if foto_bytes_exp:
                st.image(blob_to_img(foto_bytes_exp), caption=foto_label_exp, use_container_width=True)
            else:
        ...
        with dl1:
            excel_bytes_exp = exportar_ficha_excel(tramo_dict_ficha, danos_tramo_exp)
            st.download_button(
        ...
        with dl2:
            pdf_bytes_exp = exportar_ficha_pdf(tramo_dict_ficha, danos_tramo_exp)
            download_button_bytes(
        ...
        st.caption("El expediente incluye: datos generales, fotografía principal, ficha 1-B (itinerario) y ficha 1-D (daños registrados).")
