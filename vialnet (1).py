...
Persistencia: SQLite local (vialnet.db) — se crea automáticamente.
"""
...
DB_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "vialnet.db")
...
def init_db():
    ...
    """)
    # Migración suave: si la BD ya existía con las columnas antiguas de foto individuales
    # (foto_inicio / foto_intermedia / foto_final / foto_danos), se migran a tramo_fotos
    # para no perder evidencia fotográfica ya cargada por el usuario.
    try:
        cols_tramos_actual = [d[1] for d in c.execute("PRAGMA table_info(tramos)").fetchall()]
        legacy_cols = [
        ...
        if any(col for col, _ in legacy_cols if col in cols_tramos_actual):
            ya_migrado = c.execute(
                "SELECT COUNT(*) FROM tramo_fotos WHERE pie_foto LIKE '[migrado] %'"
            ).fetchone()[0]
            if ya_migrado == 0:
                cols_presentes = [col for col, _ in legacy_cols if col in cols_tramos_actual]
                if cols_presentes:
                    filas_legacy = c.execute(
                        f"SELECT id, {', '.join(cols_presentes)} FROM tramos"
                    ).fetchall()
                    for fila in filas_legacy:
                        t_id = fila[0]
                        for i, col in enumerate(cols_presentes, start=1):
                            blob_val = fila[i]
                            if blob_val:
                                etiqueta = dict(legacy_cols)[col]
                                c.execute(
                                    "INSERT INTO tramo_fotos (tramo_id, nombre_archivo, imagen, pie_foto, fecha_carga, orden) "
                                    "VALUES (?,?,?,?,?,?)",
                                    (t_id, f"{col}.jpg", blob_val, f"[migrado] {etiqueta}",
...
def guardar_fotos_tramo(tramo_id: int, fotos: list):
    """Inserta una o varias fotografías asociadas a un tramo.

    `fotos` es una lista de dicts con las llaves: nombre_archivo, imagen (bytes),
    ...
    for i, foto in enumerate(fotos[:MAX_FOTOS_TRAMO], start=1):
        c.execute(
            ...
            "VALUES (?,?,?,?,?,?)",
            (
                tramo_id, foto.get("nombre_archivo"), foto["imagen"],
...
def obtener_fotos_tramo(tramo_id: int) -> list:
    """Devuelve la lista de fotografías de un tramo, ordenadas, como lista de dicts."""
    conn = get_conn()
    filas = conn.execute(
...
def _primera_foto_disponible(fotos: list):
    """Devuelve (bytes, etiqueta) de la primera fotografía disponible de la lista de fotos del tramo."""
    if not fotos:
        return None, None
    primera = fotos[0]
    etiqueta = primera.get("pie_foto") or (primera.get("nombre_archivo") or "Fotografía del mantenimiento")
    return primera["imagen"], etiqueta
    ...
    if fotos:
        ...
        if PILImage:
            for idx, foto in enumerate(fotos[:MAX_FOTOS_TRAMO]):
                ...
                try:
                    pil_img = PILImage.open(BytesIO(foto["imagen"])).convert("RGB")
